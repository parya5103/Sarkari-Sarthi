...
## 2024-07-07 - Native Lazy Loading Above vs Below the Fold
**Learning:** Native lazy loading (`loading="lazy"`) is an excellent zero-javascript optimization for images below the initial viewport fold. However, applying it indiscriminately to images *above* the fold (like hero images or header logos) is a performance anti-pattern. It forces the browser to wait for layout before requesting the image, delaying fetching and negatively impacting Web Vitals (Specifically Largest Contentful Paint).
**Action:** When implementing lazy loading, manually verify the visual position of images. Only apply `loading="lazy"` to images explicitly verified to be located below the initial viewport fold.
