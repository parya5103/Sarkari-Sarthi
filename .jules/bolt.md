...
## 2024-05-30 - Native Image Lazy Loading Below the Fold
**Learning:** Browsers eagerly fetch all images specified in `<img>` tags during the initial page load, even if they are far down the page (below the fold). This wastes network bandwidth and can block the main thread from loading more critical Above-The-Fold resources, delaying the Largest Contentful Paint (LCP) and Page Load metrics.
**Action:** Always add the `loading="lazy"` attribute to `<img>` tags that are confirmed to be below the initial viewport fold (e.g., footer icons, feature sections). Do NOT apply this to images above the fold (like hero images), as it is a performance anti-pattern that will delay their loading and worsen LCP.
