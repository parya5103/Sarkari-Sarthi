...
## 2025-02-26 - Path Traversal Vulnerability in file paths
**Vulnerability:** Found path traversal vulnerabilities in `storage.py` and `parser.py` where externally sourced job IDs were directly used in file paths (`os.path.join(JOB_DIR, f"{job['id']}.json")`).
**Learning:** Using untrusted data like identifiers to construct file paths can allow attackers to read, overwrite, or delete arbitrary files by supplying paths like `../../`.
**Prevention:** Always sanitize externally sourced identifiers used in file paths using `os.path.basename(str(identifier))` to ensure only the filename is used and directory traversal characters are ignored.
